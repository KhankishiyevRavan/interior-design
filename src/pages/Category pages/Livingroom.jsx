...
//                   Ümumi <i className="fa-thin fa-sliders"></i>
//                 </button>
...
function Livingroom(props) {
  ...
  useEffect(() => {
    const getData = async () => {
      let data = await fetch("https://khankisiyevravan.github.io/interior-designs-fake-db/furniture.json").then((a) =>
        a.json()
      );
      setProducts(data.filter((d) => d.type == "1"));
    };
    getData();
    const getStyle = async () => {
      let data = await fetch("https://khankisiyevravan.github.io/interior-designs-fake-db/style.json").then((b) =>
        b.json()
      );
      setStyleData(data);
    };
    getStyle();
    const getTypeData = async () => {
      let data = await fetch("https://khankisiyevravan.github.io/interior-designs-fake-db/type.json").then((c) =>
        c.json()
      );
      setTypeData(data);
    };
    getTypeData();
    const getItemData = async () => {
      let data = await fetch(" https://khankisiyevravan.github.io/interior-designs-fake-db/item.json").then((a) =>
        a.json()
      );
    };
    getItemData();
